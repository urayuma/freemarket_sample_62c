...
$(document).ready(function() {
	...
	//スライド作成箇所
	let arrTitlewords = ['Find Your Way','Memory', 'Amazing life', 'Revolution', 'Keep Going', 'Everyday', 'Together']; 
	...
	let generateSlide = function(titleword) {
		...
		for (let i = 0, length = arrTitlewordsDivided[titleword].length; i < length; i++) {
			const text = 
	...
	$(document).on('mousedown touchstart', '.slide', function(e) {
		...
		$(document).on('mousemove touchmove', function(e) {
			...
			diff = startX - x;
			if (target === 1 && diff < 0 || target === numOfTitlewords && diff > 0) return;
